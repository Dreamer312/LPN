...
import yaml
import math
#from cmh_mgn_utils import load_network
...
if opt.multi:
    ...
    dataloaders = {x: torch.utils.data.DataLoader(image_datasets[x], batch_size=opt.batchsize,
                                             shuffle=False, num_workers=7) for x in ['satellite','street']}
...
def extract_feature(model, dataloaders, view_index=1, dim=None):
    # print(f"LPN {opt.LPN}")
    ...
    for data in dataloaders:
        ...
        ff_global = torch.FloatTensor(n, dim).zero_().cuda()
        ff = torch.FloatTensor(n, dim).zero_().cuda()
            ...
            for i in range(2):
                ...
                for scale in ms:
                    ...
                    if opt.views ==2:
                        ...
                        # if view_index == 1:
                        #     result = model(input_img, None)
                        #     outputs, _, = result["s4_res_logits"]

                        # elif view_index ==2:
                        #     result = model(None, input_img)
                        #     _, outputs, = result["s4_res_logits"]
                        # elif view_index ==3:
                        #     result = model(None, None, input_img)
                        #     _, _, outputs = result["s4_res_logits"]
                        #     # print(f"s4_res_logits outputs {outputs.size()}")
                        #     # assert(0)
...
print(gallery_label[:10])
print(query_label[:10])
# assert(0)
if __name__ == "__main__":
    with torch.no_grad():
        print(f"which_query {which_query}")
        query_feature = extract_feature(model, dataloaders[query_name], which_query, dim=512)
        gallery_feature = extract_feature(model, dataloaders[gallery_name], which_gallery, dim=512)
    ...
    # print(f"gallery_feature {gallery_feature.size()}")
    # print(f"query_feature {query_feature.size()}")
    # assert(0)
    ...
    if opt.LPN:
        ...
        for i in range(7):
            ...
            #assert(0)
            if i == 0:
                query_feature_ = query_feature[:,0:embedding_dim]
                gallery_feature_ = gallery_feature[:,0:embedding_dim]
                print('-------------- 1 -----------------')
            if i == 1:
                query_feature_ = query_feature[:,embedding_dim:2*embedding_dim]
                gallery_feature_ = gallery_feature[:,embedding_dim:2*embedding_dim]
                print('-------------- 2 -----------------')
            if i == 2:
                query_feature_ = query_feature[:,2*embedding_dim:3*embedding_dim]
                gallery_feature_ = gallery_feature[:,2*embedding_dim:3*embedding_dim]
                print('-------------- 3 -----------------')
            if i == 3:
                query_feature_ = query_feature[:,3*embedding_dim:4*embedding_dim]
                gallery_feature_ = gallery_feature[:,3*embedding_dim:4*embedding_dim]
                print('-------------- 4 -----------------')
            if i == 4:
                query_feature_ = query_feature[:,0:2*embedding_dim]
                gallery_feature_ = gallery_feature[:,0:2*embedding_dim]
                print('-------------- 1+2 -----------------')
            if i == 5:
                query_feature_ = query_feature[:,0:3*embedding_dim]
                gallery_feature_ = gallery_feature[:,0:3*embedding_dim]
                print('-------------- 1+2+3 -----------------')
            # if i == 6:
            #     query_feature_ = query_feature[:,0:4096]
            #     gallery_feature_ = gallery_feature[:,0:4096]
            #     print('-------------- 1+2+3+4 -----------------')
