...
from model_new import swin_infonce_region_cluster
...
class HardSampler(DistributedSampler[T_co]):
    ...
    def generate_indices_sim(self):
        ...
        hard_size = int(self.current_size * self.hard_ratio)
        ori_size = self.current_size - hard_size

        ori_list = self.current_indices[:ori_size]

        # print("self.current_size", self.current_size)
        # print("hard_size", hard_size)
        # print("ori_size", ori_size)
        # print("ori_list", len(ori_list))
        # self.current_size 40000
        # hard_size 10000
        # ori_size 30000
        # ori_list 30000
        ...
        if self.epoch >= self.mining_start:
            ...
            ori_batch_size = self.batch_size * ori_size // self.current_size
            hard_batch_size = self.batch_size - ori_batch_size

            #print("ori_batch_size", ori_batch_size) ori_batch_size 24
            #print("hard_batch_size", hard_batch_size) 8

            for i in range(self.current_size // self.batch_size):
                index_s = i * ori_batch_size
                index_e = index_s + ori_batch_size
                self.current_indices.extend(ori_list[index_s:index_e])
                hard_list = []

                #print("第一个batch采样, 采样原本的sample数量", len(self.current_indices)) # 全局batch是16*2, 第一个batch采样, 采样原本的sample数量 24

                for _ in range(hard_batch_size):
                    grd_id = random.choice(self.dataset.train_sat_cover_dict[self.dataset.train_sat_cover_list[ori_list[index_s]]])
                    ...
                    while True:
                        ...
                        for grd_idx in self.dataset.train_sat_cover_dict[self.dataset.train_sat_cover_list[idx]]:
                            if not self.dataset.check_overlap(ori_list[index_s:index_e], grd_idx) or not self.dataset.check_overlap(hard_list, grd_idx):
                                flag = True
                        if flag:
                ...
                #print("第一个batch采样, 采样hard的sample数量", len(hard_list)) #第一个batch采样, 采样hard的sample数量 8

                self.current_indices.extend(hard_list)
                #print("完成random和hard采样之后的self.current_indices", len(self.current_indices)) #32
                #assert(0)
...
def train_model(opt):
    ...
    train_sampler = mining_sampler(image_datasets, batch_size=opt.batchsize, dim=(opt.block * opt.class_dim), save_path="./")
    ...
    # ============ building networks ... ============
    model = swin_infonce_region_cluster(len(image_datasets), droprate=opt.droprate, stride=opt.stride, pool=opt.pool,
...
def train_one_epoch(accelerate, model, epoch, criterion_class, infonce, optimizer, accuracy, dataloaders, scheduler, num_epochs, opt, train_sampler):
    ...
    for data in (tqdm(dataloaders) if accelerate.is_local_main_process else dataloaders):
        ...
        y1_s4_part, y2_s4_part = result['part_logits']

        y1_s4_part = torch.stack(y1_s4_part, dim=2)
        y1_s4_part_fnorm = torch.norm(y1_s4_part, p=2, dim=1, keepdim=True) * np.sqrt(opt.block) 
        y1_s4_part = y1_s4_part.div(y1_s4_part_fnorm.expand_as(y1_s4_part))
        y1_s4_part = y1_s4_part.reshape(y1_s4_part.size(0), -1)

        y2_s4_part = torch.stack(y2_s4_part, dim=2)
        y2_s4_part_fnorm = torch.norm(y2_s4_part, p=2, dim=1, keepdim=True) * np.sqrt(opt.block) 
        y2_s4_part = y2_s4_part.div(y2_s4_part_fnorm.expand_as(y2_s4_part))
        y2_s4_part = y2_s4_part.reshape(y2_s4_part.size(0), -1)  # 16*4608

        features = torch.cat([y1_s4_part.unsqueeze(1), y2_s4_part.unsqueeze(1)], dim=1)
        loss_infonce = infonce(features, all_label)
        loss_main = loss_main + loss_infonce



        sate_embd_all = accelerate.gather(y1_s4_part)
        street_embd_all = accelerate.gather(y2_s4_part)
